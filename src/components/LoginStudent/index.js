...
class LoginStudent extends Component {
  ...
  checkStudentAccount = async () => {
    ...
    if (checkPreviousResponse.status === 200) {
      ...
      await this.setState({
        fetchState: "tryAgain",
        err: "You are not a Student, Use Student account to Login",
      });
    }
  };
  ...
  getData = async (setNameParent, person) => {
    ...
    if (checkPreviousResponse.status === 200) {
      ...
      Cookies.set("student_token", `Student ${name}`);
  ...
  submitForm = async (event) => {
    ...
    if (userData.every((item) => item !== "")) {
      const checkStudent = await this.checkStudentAccount();
      if (checkStudent) {
        this.getData();
      } else {
        this.setState({
          err: "This Is Master account. Please Enter Student Account",
        });
      }
    } else {
